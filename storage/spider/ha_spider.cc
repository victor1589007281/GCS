...
THR_LOCK_DATA **ha_spider::store_lock(
  ...
  if (lock_type != TL_IGNORE && lock.type == TL_UNLOCK)
  {
      ...
      (spider_param_lock_exchange(thd) == 1 && share->semi_table_lock))
    {
      ...
      ) { // local_lock_table 默认为 true, 此逻辑不走
        for (
...
int ha_spider::external_lock(
  ...
  if ((conn_kinds & SPIDER_CONN_KIND_MYSQL))
  {
	...
	{//  若是在执行SQL的时候创建连接， 则此处conns为空，会报错
    ...
    (HA_STATUS_TIME | HA_STATUS_CONST | HA_STATUS_VARIABLE | HA_STATUS_AUTO))
  {
    ...
    if (flag & HA_STATUS_AUTO)
    {
      tmp_auto_increment_mode = spider_param_auto_increment_mode(thd,
		...
		if (spider_param_spider_get_sts_or_crd() && 
        (sql_command == SQLCOM_SHOW_TABLE_STATUS || sql_command == SQLCOM_SHOW_KEYS) &&
        difftime(tmp_time, share->crd_get_time) >= crd_interval && sts_crd_errornum_flag 
        )
		{ // 计算间隔时间 sts_interval是通过global variables spider_sts_interval来指定
			int roop_count; // 设置此处的conns为null，防止上次操作过程conn在free后未将此值值NULL
			for (
...
#if defined(HA_CAN_BULK_ACCESS) && defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
...
int ha_spider::close_opened_handler(
  ...
  {
	  SPIDER_CONN *spider_conn = spider_get_conn_by_idx(link_idx);
		if(!spider_conn)
		{
			error_num = ER_SPIDER_CON_COUNT_ERROR;
			DBUG_RETURN(error_num);
    ...
    if ((error_num2 = spider_db_close_handler(this,
      spider_conn, link_idx, SPIDER_CONN_KIND_MYSQL))
    ) {
      if (
    ...
    spider_clear_bit(m_handler_opened, link_idx);
		if (release_conn)
		{
			...
			spider_free_conn_from_trx(trx, spider_conn, FALSE, FALSE, NULL);
			conns[link_idx] = NULL;
		}
  }
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
...
#endif
#endif
				SPIDER_CONN *spider_conn = spider_get_conn_by_idx(roop_count);
				if(!spider_conn)
				{
					error_num = ER_SPIDER_CON_COUNT_ERROR;
					DBUG_RETURN(error_num);
...
void ha_spider::check_pre_call(
  ...
  if (!spider_param_use_pre_scan() || 
        (thd->lex && thd->lex->sql_command != SQLCOM_SELECT))  // 如 insert .. select ..
  {
    use_pre_call = FALSE;
    DBUG_VOID_RETURN;
  }
