...
int spider_db_query(
  ...
  DBUG_ENTER("spider_db_query");
	thd_proc_info(0,"spider_db_query start");
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
  if (conn->conn_kind == SPIDER_CONN_KIND_MYSQL)
  {
#endif
    ...
    if ((error_num = conn->db_conn->exec_query(query, length, quick_mode)))
      DBUG_RETURN(error_num);
		thd_proc_info(0,"spider_db_query end");
    DBUG_RETURN(0);
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
  } else {
    if (conn->queued_net_timeout)
...
int spider_db_show_table_status(
  ...
  int error_num;
	spider_db_handler *dbton_hdl = NULL;
...
int spider_db_update_auto_increment(
  ...
  if (
    ...
    (auto_increment_mode == 3 && !table->auto_increment_field_not_null)
	  ...
	  SPIDER_CONN *spider_conns = spider->spider_get_conn_by_idx(link_idx);
		if(!spider_conns)
		{
			DBUG_RETURN(ER_SPIDER_CON_COUNT_ERROR);
		}
    last_insert_id = spider_conns->db_conn->last_insert_id();
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
    if (spider->conn_kind[link_idx] == SPIDER_CONN_KIND_MYSQL)
    {
#endif
      affected_rows = spider_conns->db_conn->affected_rows();
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
    } else {
...
int spider_db_bulk_direct_update(
  ...
  for (
    ...
    if (!spider_bit_is_set(spider->do_hs_direct_update, roop_count))
    {
      ...
      if (!counted)
      {
        *update_rows = spider->spider_get_conn_by_idx(roop_count)->db_conn->affected_rows();
        DBUG_PRINT("info", ("spider update_rows = %u", *update_rows));
        counted = TRUE;
      }
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
    ...
    if (spider->is_bulk_access_clone)
    {
      ...
      if (!spider_bit_is_set(spider->do_hs_direct_update, roop_count))
      {
				...
				if (!counted)
				{
					...
					*delete_rows = spider_conn->db_conn->affected_rows();
...
)
{
    ...
    return spider_db_print_item_type(item, spider, str, alias, alias_length, dbton_id, field_charset);
}
...
{
    return spider_db_print_item_type(item, spider, str, alias, alias_length, dbton_id, NULL);
}
...
int spider_db_open_item_func(
  ...
  DBUG_RETURN(spider_dbton[dbton_id].db_util->open_item_func(
    item_func, spider, str, alias, alias_length, field_charset));
}
...
int spider_db_open_item_sum_func(
  ...
  DBUG_RETURN(spider_dbton[dbton_id].db_util->open_item_sum_func(
    item_sum, spider, str, alias, alias_length, field_charset));
}
#endif
  ...
  switch (item_cache->result_type())
  {
    case STRING_RESULT:
      DBUG_RETURN(spider_db_open_item_string(item_cache, spider, str,
        alias, alias_length, dbton_id, NULL));
    case ROW_RESULT:
      {
        int error_num;
...
/*
    ...
    如果是binary/varbinary/char/varchar/blob/text等类型，返回其字符集

    否则返回NULL

*/
CHARSET_INFO*
spider_get_item_field_charset(
    Item* item,
    ha_spider* spider
)
{
    ...
    case Item::FIELD_ITEM:
    {
        ...
        if (item_field->result_type() == STRING_RESULT)
        {
            ...
            // 如果collation_connection与表字符集一致，就不需检查列字符集
...
int spider_db_append_update_columns(
  ...
  while ((field = fi++))
  {
    ...
    if ((error_num = spider_db_print_item_type(
      (Item *) value, spider, str, alias, alias_length, dbton_id, spider_get_item_field_charset(field, spider))))
      DBUG_RETURN(error_num);
    if (str)
    {
      if (str->reserve(SPIDER_SQL_COMMA_LEN))
  ...
  while (!end_of_file)
  {
    ...
    if (copy_tables->use_transaction)
    {
      for (roop_count = 0; roop_count < all_link_cnt; roop_count++)
      {
        ...
        tmp_conn = tmp_spider->spider_get_conn_by_idx(0);
