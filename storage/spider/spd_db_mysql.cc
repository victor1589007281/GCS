...
int spider_db_mysql::set_net_read_write_time(uint net_read_timeout, uint net_write_timeout)
{
	if(!db_conn)
		return 0;
...
int spider_db_mysql_util::open_item_func(
    ...
    case Item_func::UNKNOWN_FUNC:
      ...
      } else if (func_name_length == 17)
      {
        if (!strncasecmp("date_add_interval", func_name, func_name_length))
        {
          ...
          func_name_length = strlen(func_name);
          if ((error_num = spider_db_print_item_type_and_check_charset(item_list[0], spider, str,
            alias, alias_length, dbton_id, field_charset)))
            DBUG_RETURN(error_num);
          if (str)
          {
          ...
          if ((error_num = spider_db_print_item_type_and_check_charset(item_list[1], spider, str,
            alias, alias_length, dbton_id, field_charset)))
            DBUG_RETURN(error_num);
          if (str)
          {
  ...
  if (item_func->functype() == Item_func::FT_FUNC)          //fulltext
  {
...
int spider_mysql_handler::append_sum_select(
  ...
  for (item_sum_ptr = join->sum_funcs; *item_sum_ptr; ++item_sum_ptr)
  {
    if ((error_num = spider_db_mysql_utility.open_item_sum_func(*item_sum_ptr,
      spider, str, alias, alias_length, NULL)))
    {
      DBUG_RETURN(error_num);
    }
    if (str->reserve(SPIDER_SQL_COMMA_LEN))
...
int spider_mysql_handler::append_from(
  ...
  if (sql_type == SPIDER_SQL_TYPE_HANDLER)
  {
    ...
    if(opt_spider_index_hint_pushdown)
...
int spider_mysql_handler::show_table_status(
  ...
  uint flag
	...
	if (sts_mode == 1)
	{
		uint net_read_timeout_tmp;
		uint net_write_timeout_tmp;
		uint connect_timeout_tmp;
