...
static my_bool  verbose= 0, opt_no_create_info= 0, opt_no_data= 0,
		...
		foreign_key_check= 0,
                ...
                opt_alltspcs=0, opt_notspcs= 0,opt_enable_compress_optimization = 0;
...
static char  *opt_password=0,*current_user=0,
             ...
             *err_ptr= 0,
             ...
             /* harryczhang: Path of splitted .gz files from mysqldump. */
             *gzpath = 0,
...
/*#define my_snprintf(buf, n, fmt, arg) \
    do { \
      int ret = sprintf((buf), (n), (fmt), (arg)); \
      if(ret < 0 || ret >= n) { \
        fprintf(stderr, "my_snprintf is error."); \
      } \
    } while(0)
    */

/* harryczhang: used for multiple dump. */
typedef struct {
...
#define MYSQL_OPT_SPLIT_COUNT_MAX_VALUE 128
...
FILE *META = 0;
ZFILE dump_begin_file = 0;
ZFILE dump_end_file = 0;
char *global_buffer = 0;

static ZFILE my_open_zip(const char* file_full_name);
static void my_close_zip(ZFILE file);
static int my_gzputc(int c, gzFile file);
static int my_gzputs(const char* s, gzFile file);
int my_gzprintf(gzFile file, const char* format, ...) ;

#ifdef __WIN__
#define ZPUTC my_gzputc
...
static ZFILE open_sql_zip_file_for_table(const char *db, const char *table, const char *type, size_t piece_id);
static my_bool z_get_view_structure(char *table, char* db);
static int z_dump_trigger(ZFILE sql_file, MYSQL_RES *show_create_trigger_rs,
                        const char *db_name,
                        const char *db_cl_name);
static void z_dump_trigger_old(ZFILE sql_file, MYSQL_RES *show_triggers_rs,
                             MYSQL_ROW *show_trigger_row,
                             const char *table_name);
static void z_unescape(ZFILE file,char *pos,uint length);
static void z_print_blob_as_hex(ZFILE output_file, const char *str, ulong len);
static uint z_get_table_structure(char *table, char *db, char *table_type,
                                char *ignore_flag, my_bool *is_blob_compress_in_table);
static uint z_dump_routines_for_db(char *db);
static int z_restore_db_collation(ZFILE sql_file,
                                const char *db_name,
                                const char *delimiter,
                                const char *db_cl_name);
static void z_restore_cs_variables(ZFILE sql_file,
                                 const char *delimiter);
static void z_restore_sql_mode(ZFILE sql_file,
                             const char *delimiter);
static void z_restore_time_zone(ZFILE sql_file,
                              const char *delimiter);
static void z_switch_time_zone(ZFILE sql_file,
                             const char *delimiter,
                             const char *time_zone);
static void z_switch_sql_mode(ZFILE sql_file,
                            const char *delimiter,
                            const char *sql_mode);
static void z_switch_cs_variables(ZFILE sql_file,
                                const char *delimiter,
                                const char *character_set_client,
                                const char *character_set_results,
                                const char *collation_connection);
static int z_switch_db_collation(ZFILE sql_file,
                               const char *db_name,
                               const char *delimiter,
...
/* @harryczhang: default dbug output under working directory. */
/* const char *default_dbug_option="d:t:o,/tmp/mysql.trace"; */
const char *default_dbug_option="d:t:o,tmysql.trace"; 
...
static struct my_option my_long_options[] =
{
  ...
  {"delete-master-logs", OPT_DELETE_MASTER_LOGS,
   ...
   GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
  {"enable-compress-optimization", OPT_ENABLE_COMPRESS_OPTIMIZATION,
  "Use to forbid Importing and exporting Optimization for blob compressed data.",
  &opt_enable_compress_optimization, &opt_enable_compress_optimization, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
  {"disable-keys", 'K',
...
#include <sslopt-longopts.h>
  ...
  {"gztab",'G',
   "Create separated compressed sqlfile for each table to given path."
   "Will NOT dump any TABLESPACE when use this opt.",
   &gzpath, &gzpath, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},

   /* harryczhang: add two opts to control segmentation of large tables. */
  {"largetable_size",                       /* option name */
   0,                                       /* no short version */
                                            /* comment */
...
/*!40014 SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=%d */;\n\
/*!40014 SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=%d */;\n\
", unique_key_check?1:0, foreign_key_check?1:0);
    }
    fprintf(sql_file,
...
static int get_options(int *argc, char ***argv)
{
  ...
  *mysql_params->p_net_buffer_length= opt_net_buffer_length;
  ...
  if(gzpath && opt_alltspcs)
  {
    fprintf(stderr, "%s: You can't use --gztab to dump any tablespace\n", my_progname);
    return(EX_USAGE);
  }
...
static void free_resources()
{
  ...
  /* harryczhang: close these files resources */
  if (stderror_file)
    my_fclose(stderror_file, MYF(MY_WME));
  if (dump_begin_file)
    my_close_zip(dump_begin_file);
  if (dump_end_file)
    my_close_zip(dump_end_file);
  if (META)
    my_fclose(META, MYF(MY_WME));
  if (global_buffer)
    my_free(global_buffer);
  my_end(my_end_arg);
}
                                ...
                                char *ignore_flag, my_bool *is_blob_compress_in_table)
{
  ...
  *ignore_flag= check_if_ignore_table(db, table, table_type); // table_type is output parameter
  ...
  else
  {
	...
	if (opt_enable_compress_optimization)
	{
		opt_enable_compress_optimization = 0;
		verbose_msg("disable compress optimization\n",
			my_progname);
	}
...
static int dump_triggers_for_table(char *table_name, char *db_name)
{
  ...
  if(gzpath)
  {
    zsql_file = open_sql_zip_file_for_table(db_name, table_name, "trigger");
    if (zsql_file == NULL)
    {
	fprintf(stderr, "Creat trigger file for %s.%s failed\n", db_name, table_name);
	exit(1);
    }
    z_write_header(zsql_file, db_name);
    z_print_comment(zsql_file, 0,
                  "\n--\n-- Current Database: %s\n--\n", db_name);
  ...
  while ((row= mysql_fetch_row(show_triggers_rs)))
  {
    ...
    if (mysql_query(mysql, query_buff))
    {
      ...
      */
      ...
      if(gzpath)
      {
	  z_dump_trigger_old(zsql_file, show_triggers_rs, &row, table_name);
      }
      else 
      {
    ...
    else
    {
	  ...
	  if (!show_create_trigger_rs ||
	      z_dump_trigger(zsql_file, show_create_trigger_rs, db_name, db_cl_name))
	    goto done;
      }
      else
      {
...
skip:
  ...
  */
  ...
  if (gzpath)
  {
    z_write_footer(zsql_file);
    my_close_zip(zsql_file);
  }
...
static my_ulonglong get_bytes_by_str(const char *src) {
  ...
  for (; *beg && isdigit(*beg); ++beg)
    ;
  len = beg - src;
  if (!len) {
    /* empty src or the first letter of src is not digit, such as +120G */
    return err_ret;
  }
  if (len > 31) {
    /* stack overflow */
    ...
    const char *db_name, const char *table_name)
{
  my_bool err_ret = 1;
  my_bool suc_ret = 0;
  char buff[FN_REFLEN + 1024];
  char show_name_buff[FN_REFLEN];
  MYSQL_RES *res = NULL;
  MYSQL_ROW row;
  uint field_num = 0;

  DBUG_ENTER("check_table_size");
  DBUG_ASSERT(table_status != NULL);
  ...
  if (mysql_get_server_version(mysql) >= FIRST_INFORMATION_SCHEMA_VERSION) {
    ...
    if (my_snprintf(buff, sizeof(buff), "show table status like %s",
          quote_for_like(table_name, show_name_buff)) >= sizeof(buff)) {
      goto err;
    }
  }
  ...
  if (field_num >= 18) {
    ...
    Max_data_length: 0
       Index_length: 0
          Data_free: 9437184
     Auto_increment: NULL
        Create_time: 2013-12-11 10:59:19
        Update_time: NULL
         Check_time: NULL
...
static void dump_table(char *table, char *db)
{
  ...
  */
  if(gzpath)
  {
    num_fields= z_get_table_structure(table, db, table_type, &ignore_flag, &is_blob_compress_in_table);
  }
  else
  {
    num_fields= get_table_structure(table, db, table_type, &ignore_flag, &is_blob_compress_in_table);
  }
...
//      }
  ...
  if (opt_routines && mysql_get_server_version(mysql) >= 50009)
  {
    ...
    if(gzpath)
	    z_dump_routines_for_db(database);
    else
...
//  }
  ...
  while ((table= getTableName(0)))
  {
	...
	if(gzpath)
	{
          z_get_view_structure(table, database);
	}
	else
	{
...
//    if(!(stderror_file= freopen(log_error_file, "a+", stderr)))
    if(!(stderror_file = my_freopen(log_error_file, "a+", stderr)))
    {
      free_resources();
      exit(EX_MYSQLERR);
    }
  ...
  if(gzpath)
  {
    if (my_snprintf(tmp_buf, sizeof(tmp_buf), "%s", gzpath) >= sizeof(tmp_buf))
    {
      fprintf(stderr, "gzpath specified by --gztab is too long.\n");
      free_resources();
      exit(EX_MYSQLERR);
    }
    ...
    if (my_snprintf(file_extend, sizeof(file_extend), "%s", ".BEGIN.sql.gz") >= sizeof(file_extend))
    {
      fprintf(stderr, "file_extend's size is not enough for dump_begin_file.\n");
      free_resources();
      exit(EX_MYSQLERR);
    }
    ...
    if (my_snprintf(meta_path, sizeof(meta_path), "%s/META", tmp_path) >= sizeof(meta_path))
    {
      fprintf(stderr, "meta_path's size is not enough\n");
      free_resources();
      exit(EX_MYSQLERR);
    }
...
//    global_buffer = (char*)malloc(sizeof(char)*GBUFF);
    global_buffer = (char*)my_malloc(sizeof(char) * GBUFF, MYF(MY_WME));
    if (!global_buffer)
    {
      goto err;
    }
  ...
  if(gzpath)
    z_write_header(dump_begin_file, *argv);
  ...
  */
  ...
  if (opt_slave_apply)
  {
    if (gzpath)
    {
      if (z_add_slave_statements())
        goto err;
    }
    else
    {
      if (add_slave_statements())
        goto err;
    }
  }
...
//    my_fclose(stderror_file, MYF(MY_WME));
  ...
  if(gzpath)
  {
    ...
    {
      dump_end_file = my_open_zip(fn_format(filename, "DUMP", tmp_path, file_extend, 4));
      if (dump_end_file)
        z_write_footer(dump_end_file);
      else
        fprintf(stderr, "Create dump_end_file error.\n");
    }
  }
...
static ZFILE my_open_zip(const char* file_full_name)/*{{{*/
{
  ZFILE res = 0;
#ifndef __WIN__
  char pline[FN_REFLEN];
#endif

  DBUG_ENTER("my_open_zip");
#ifdef __WIN__
  res = gzopen(file_full_name, "w");
  if(!res)
  {
    fprintf(stderr, "Crerat %s failed\n", file_full_name);
    DBUG_RETURN(NULL);
  }
#else
  snprintf(pline, sizeof(pline), "gzip >%s", file_full_name);
  res = popen(pline, "w");
  if(!res)
  {
    fprintf(stderr, "Open pipe failed on %s\n", pline);
    DBUG_RETURN(NULL);
  }
#endif
  DBUG_RETURN(res);
...
static ZFILE open_sql_zip_file_for_table(const char* db, const char* table, const char* type)/*{{{*/
{
    ...
    my_snprintf(file_extend, sizeof(file_extend), ".%s.sql.gz", type);
    res = my_open_zip(fn_format(file_path, table, path_add_db, file_extend, 4));
    snprintf(for_meta_fn, sizeof(for_meta_fn), "%s/%s.%s.sql.gz", db, table, type);
    fprintf(META, "%s\t%s\t%s\t%s\n", db, table, type, for_meta_fn);
    DBUG_RETURN(res);
}/*}}}*/

/* harryczhang: multiple splitted pieces */
static ZFILE open_sql_zip_file_for_table(const char *db,
    const char *table,
    const char *type,
    size_t piece_id)
{
...
#  ifdef HAS_vsprintf_void
    (void)vsprintf(global_buffer, format, va);
    va_end(va);
    len = strlen(global_buffer);
#  else
    len = vsprintf(global_buffer, format, va);
    va_end(va);
#  endif
#else
#  ifdef HAS_vsnprintf_void
    (void)vsnprintf(global_buffer, GBUFF, format, va);
    va_end(va);
    len = strlen(global_buffer);
#  else
    len = vsnprintf(global_buffer, GBUFF, format, va);
    va_end(va);
#  endif
#endif
    if (len <= 0 || len >= GBUFF || global_buffer[GBUFF - 1] != 0)
        return 0;
    return gzwrite(file, global_buffer, (unsigned)len);
}/*}}}*/
...
static void z_write_header(ZFILE sql_file, const char *db_name)/*{{{*/
{
  if (!opt_compact)
  {
    z_print_comment(sql_file, 0,
                  ...
                  DUMP_VERSION, MYSQL_SERVER_VERSION, SYSTEM_TYPE,
                  MACHINE_TYPE);
    z_print_comment(sql_file, 0, "-- Host: %s    Database: %s\n",
                  current_host ? current_host : "localhost",
                  db_name ? db_name : "");
    z_print_comment(sql_file, 0,
                  "-- ------------------------------------------------------\n"
                 );
    z_print_comment(sql_file, 0, "-- Server version\t%s\n",
                  mysql_get_server_info(&mysql_connection));
...
"\n/*!40101 SET NAMES %s */;\n",default_charset);
    ...
    if (opt_tz_utc)
    {
      ...
      ZPRINTF(sql_file, "/*!40103 SET TIME_ZONE='+00:00' */;\n");
    }
    ZPRINTF(sql_file,"\
/*!40014 SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=%d */;\n\
/*!40014 SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=%d */;\n\
...
static void z_write_footer(ZFILE sql_file)/*{{{*/
{
  ...
  {
    if (opt_tz_utc)
      ZPRINTF(sql_file,"/*!40103 SET TIME_ZONE=@OLD_TIME_ZONE */;\n");
...
"/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;\n");
    ...
    if (opt_dump_date)
    {
      char time_str[20];
      get_date(time_str, GETDATE_DATE_TIME, 0);
      z_print_comment(sql_file, 0, "-- Dump completed on %s\n", time_str);
    }
    else
      z_print_comment(sql_file, 0, "-- Dump completed\n");
...
static int z_do_show_slave_status(MYSQL *mysql_con)/*{{{*/
{
  ...
  const char *comment_prefix=
    (opt_slave_data == MYSQL_OPT_SLAVE_DATA_COMMENTED_SQL) ? "-- " : "";
  DBUG_ENTER("z_do_show_slave_status");
  if (mysql_query_with_error_report(mysql_con, &slave, "SHOW SLAVE STATUS"))
  {
    if (!ignore_errors)
    {
...
static int z_add_slave_statements(void)
{
  DBUG_ENTER("z_add_slave_statements");
  if (opt_comments)
    ZPRINTF(dump_end_file,
            "\n--\n-- start slave statement to make a recovery dump)\n--\n\n");
  ZPRINTF(dump_end_file, "START SLAVE;\n");
  DBUG_RETURN(0);
}
...
static uint z_dump_events_for_db(char *db)/*{{{*/
{
  ...
  sprintf(file_extend, ".%s.sql.gz", db);
  convert_dirname(tmp_path, gzpath, NullS);
  sql_file = my_open_zip(fn_format(filename, "events", tmp_path, file_extend, 4));
  if(sql_file == NULL)
  {
...
//    exit(1);
  ...
  */
  ...
  if (mysql_num_rows(event_list_res) > 0)
  {
    ZPRINTF(sql_file, "/*!50106 SET @save_time_zone= @@TIME_ZONE */ ;\n");
      ...
      while ((row= mysql_fetch_row(event_res)) != NULL)
      {
        ...
        */
        if (strlen(row[3]) != 0)
        {
          ...
          if (mysql_num_fields(event_res) >= 7)
          {
            if (z_switch_db_collation(sql_file, db_name_buff, delimiter,
                                    db_cl_name, row[6], &db_cl_altered))
            {
              DBUG_RETURN(1);
            }

            z_switch_cs_variables(sql_file, delimiter,
                                row[4],   /* character_set_client */
                                row[4],   /* character_set_results */
                                row[5]);  /* collation_connection */
          }
          ...
          z_switch_sql_mode(sql_file, delimiter, row[1]);

          z_switch_time_zone(sql_file, delimiter, row[2]);
          ...
          z_restore_time_zone(sql_file, delimiter);
          z_restore_sql_mode(sql_file, delimiter);

          if (mysql_num_fields(event_res) >= 7)
          {
            z_restore_cs_variables(sql_file, delimiter);
    ...
    ZPRINTF(sql_file, "DELIMITER ;\n");
    ZPRINTF(sql_file, "/*!50106 SET TIME_ZONE= @save_time_zone */ ;\n");
...
static uint z_dump_routines_for_db(char *db)/*{{{*/
{
  ...
  sprintf(file_extend, ".%s.sql.gz", db);
  convert_dirname(tmp_path, gzpath, NullS);
  sql_file = my_open_zip(fn_format(filename, "routines", tmp_path, file_extend, 4));
  if(sql_file == NULL)
  {
    fprintf(stderr, "Creat %s failed\n", filename);
    exit(1);
  ...
  sprintf(for_meta_fn, "routines.%s.sql.gz",db);
  fprintf(META, "%s\t%s\t%s\t%s\n", db, "NULL", "routines", for_meta_fn);
      ...
      while ((routine_list_row= mysql_fetch_row(routine_list_res)))
      {
        ...
        while ((row= mysql_fetch_row(routine_res)))
        {
          ...
          */
          ...
          {
            if (opt_drop)
              ZPRINTF(sql_file, "/*!50003 DROP %s IF EXISTS %s */;\n",
                      routine_type[i], routine_name);
            ...
            if (mysql_num_fields(routine_res) >= 6)
            {
              if (z_switch_db_collation(sql_file, db_name_buff, ";",
                                      db_cl_name, row[5], &db_cl_altered))
              {
                DBUG_RETURN(1);
              }

              z_switch_cs_variables(sql_file, ";",
                                  row[3],   /* character_set_client */
                                  row[3],   /* character_set_results */
                                  row[4]);  /* collation_connection */
            }
            ...
            z_switch_sql_mode(sql_file, ";", row[1]);
            ...
            z_restore_sql_mode(sql_file, ";");

            if (mysql_num_fields(routine_res) >= 6)
            {
              z_restore_cs_variables(sql_file, ";");

              if (db_cl_altered)
              {
                if (z_restore_db_collation(sql_file, db_name_buff, ";", db_cl_name))
                  DBUG_RETURN(1);
              }
            }
        ...
        if (mysql_num_rows(result))
        {
          if (opt_drop)
          {
            ...
            */
            ...
            ZPRINTF(sql_file, "/*!50001 DROP VIEW IF EXISTS %s*/;\n",
                    opt_quoted_table);
            check_io(sql_file);
          }
          ...
          */
          ...
          ZPRINTF(sql_file, "  %s %s", quote_name(row[0], name_buff, 0),
                  row[1]);
...
static void z_print_blob_as_hex(ZFILE output_file, const char *str, ulong len)
{
    ...
    const char *ptr= str, *end= ptr + len;
    for (; ptr < end ; ptr++)
      ZPRINTF(output_file, "%02X", *((uchar *)ptr));
    check_io(output_file);
}
static void z_unescape(ZFILE file,char *pos,uint length)
{
                             ...
                             const char *table_name)
{
  ...
  if (opt_compact)
    ZPRINTF(sql_file, "/*!50003 SET @OLD_SQL_MODE=@@SQL_MODE*/;\n");
  ...
  if (opt_compact)
    ZPRINTF(sql_file, "/*!50003 SET SESSION SQL_MODE=@OLD_SQL_MODE */;\n");
                        ...
                        const char *db_cl_name)
{
  ...
  while ((row= mysql_fetch_row(show_create_trigger_rs)))
  {
    ...
    query_str= cover_definer_clause(row[2], strlen(row[2]),
                                    ...
                                    C_STRING_WITH_LEN("50003"),
                                    C_STRING_WITH_LEN(" TRIGGER"));
    if (z_switch_db_collation(sql_file, db_name, ";",
                            db_cl_name, row[5], &db_cl_altered))
      DBUG_RETURN(TRUE);
    ...
    if (db_cl_altered)
    {
      if (z_restore_db_collation(sql_file, db_name, ";", db_cl_name))
        DBUG_RETURN(TRUE);
    }
...
static my_bool z_get_view_structure(char *table, char* db)/*{{{*/
{
  ...
  sql_file = open_sql_zip_file_for_table(db, table, "view");
  if(sql_file == NULL)
  {
...
//    exit(1);
  ...
  /* Table might not exist if this view was dumped with --tab. */
  ZPRINTF(sql_file, "/*!50001 DROP TABLE IF EXISTS %s*/;\n", opt_quoted_table);
  if (opt_drop)
  {
    ZPRINTF(sql_file, "/*!50001 DROP VIEW IF EXISTS %s*/;\n",
            opt_quoted_table);
