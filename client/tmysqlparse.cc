...
typedef struct result_output_audit
{
	int len;
	int maxlen;
	result_audit *ra;
}result_output_audit;
static result_output_audit roa;
...
static void tmysqlparse_result_init(result_output_audit *roa)
{
	//allocate buffer for result_audit
	roa->len = 0;
	roa->maxlen = RESULT_OUTPUT_AUDIT_LEN;
	roa->ra = (result_audit *)calloc(RESULT_OUTPUT_AUDIT_LEN, sizeof(result_audit));
}
static void tmysqlparse_result_destory(result_output_audit *roa)
{
	if(roa->ra)
	{
		...
		for(i=0; i<roa->len; ++i)
		{
			if((roa->ra)[i].sql)
			free((roa->ra)[i].sql);
			if ((roa->ra)[i].pra.table_arr)
			{

				free((roa->ra)[i].pra.table_arr);
				(roa->ra)[i].pra.table_arr = NULL;
				(roa->ra)[i].pra.n_tables_alloced = (roa->ra)[i].pra.n_tables = 0;
			}
			roa->ra->sql = NULL;
		}
		roa->len = 0;
...
static void tmysqlparse_add_pra(result_output_audit *roa, char *sql, parse_result_audit *pra)
{
	DBUG_ASSERT(roa->len <= roa->maxlen);
	if(roa->len == roa->maxlen)
	{
		result_audit *new_ra;
		new_ra = (result_audit*)calloc(roa->maxlen*2, sizeof(result_audit));
		memcpy(new_ra, roa->ra, sizeof(result_audit)*roa->maxlen);
		free(roa->ra);
		roa->ra = new_ra;
	...
	(roa->ra)[roa->len].pra.table_arr = NULL;
	(roa->ra)[roa->len].pra.thd_org = NULL;
	if(pra->tbdb == 0 && pra->result_type == 1)
	{//risk warnings, about table
		(roa->ra)[roa->len].pra.table_arr = (parse_table_struct*)calloc(pra->n_tables_alloced,
			sizeof(parse_table_struct));
		memcpy((roa->ra)[roa->len].pra.table_arr, pra->table_arr, sizeof(parse_table_struct) * pra->n_tables_alloced);	
	}
	roa->len = roa->len + 1;
}
...
static void tmysqlparse_print_xml(result_output_audit *roa, FILE *xml_file)
{
	...
	for(int i=0; i<roa->len; ++i)
	{
		...
		else if((roa->ra)[i].pra.result_type == 2)
		{
			if(flag == 1)
				flag = 3;
			else
	...
	{//parse fail
		...
		for(int i=0; i<roa->len; ++i)
		{
			if((roa->ra)[i].pra.result_type == 2)
			{
				...
				fputs("\t\t\t<error_code>",xml_file);
				fprintf(xml_file,"%d",(roa->ra)[i].pra.errcode);
				fputs("</error_code>\n",xml_file);

				fputs("\t\t\t<error_msg>",xml_file);
	...
	{//risk warnings
		...
		for(int i=0; i<roa->len; ++i)
		{
			if((roa->ra)[i].pra.result_type == 1)
			{
				...
				fputs("\t\t\t<name>",xml_file);
				if((roa->ra)[i].pra.tbdb == 0)
				{//output table name

					for (int j = 0; j < (roa->ra)[i].pra.n_tables; ++j)
					{
						print_quoted_xml(xml_file,(roa->ra)[i].pra.table_arr[j].dbname,
							strlen((roa->ra)[i].pra.table_arr[j].dbname));
						fputs(".",xml_file);					
						print_quoted_xml(xml_file,(roa->ra)[i].pra.table_arr[j].tablename,
							strlen((roa->ra)[i].pra.table_arr[j].tablename));
						if(j != (roa->ra)[i].pra.n_tables-1)
							fputs(", ",xml_file);
					}
				}
				else
