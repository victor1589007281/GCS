...
CREATE TABLE IF NOT EXISTS alter_log ( record_time timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, user_host mediumtext NOT NULL, query_time time, lock_time time, db varchar(64) NOT NULL, table_name varchar(64) NOT NULL, engine varchar(64) NOT NULL, row_format varchar(10) NOT NULL, is_partitoned int NOT NULL, sql_text mediumtext NOT NULL, affected_rows bigint NOT NULL,  data_length bigint unsigned NOT NULL, index_length bigint unsigned NOT NULL, data_free bigint unsigned NOT NULL, table_rows bigint unsigned NOT NULL, KEY IND_TIME(record_time)) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='Alter log';
